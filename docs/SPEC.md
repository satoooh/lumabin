...
- 最終更新日: 2026-05-04
- ステータス: Active（MVP実装済み / Beta hardening実施済み）
- 参照: [PRD.md](PRD.md), [ARCHITECTURE.md](ARCHITECTURE.md)

## 1. 目的

Cloudflare R2 上の画像・動画・PDF・CSV を、macOS デスクトップで高速かつ直感的に管理する。
...
補足:
- generic S3 は「S3互換 API の core operation」を対象とする。
- provider固有機能は Beta の保証範囲外とし、互換差分は RUNBOOK のトラブルシュートで吸収する。
...
実装タスク、未決事項、改善要求は GitHub Issues で管理します。SPEC には合意済みの要件と受け入れ基準だけを残します。
