...
- `LUMABIN_APPLE_CERTIFICATE_BASE64`（署名有効時は必須。Developer ID Application 証明書 `.p12` を base64 化した GitHub Secret）
- `LUMABIN_APPLE_CERTIFICATE_PASSWORD`（署名有効時は必須。上記 `.p12` の export password）
- `LUMABIN_APPLE_ID`
